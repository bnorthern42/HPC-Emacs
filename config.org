#+TITLE: Master HPC C++ Configuration
...
Visuals & UI
A clean, high-contrast dark theme for long nights debugging MPI deadlocks.
...
Session Management
Restore open files, window layouts, and cursor positions from the last session.
...
Modal Editing (The Hybrid Approach)
You like Vim keys but want a custom experience. evil-mode is the engine, but we'll use general.el to map things in a way that feels consistent across the editor.
...
Rainbow Delimiters
Colorize nested parentheses and brackets for better readability.
...
HPC projects are often defined by their root. Projectile will find your Makefile or meson.build and automatically set the compilation and run paths.
...
Modern C++ Development (LSP)
We use Eglot (built into Emacs 29+) because it is lightweight and doesn't reinvent the wheel. Ensure clangd is installed on your system.
...
HPC Debugging (GDB, OpenMP, MPI)
This is the core of your request. dape (Debug Adapter Protocol for Emacs) is far superior to GDB-MI for modern C++.

** Prerequisites
You will need a Debug Adapter for GDB. I recommend gdb with its integrated machine interface or lldb-vscode.
...
We use vterm for a full-featured terminal emulator and vterm-toggle to show/hide it at the bottom, similar to VS Code.
...
Completion System
Using the "Vertical" stack (Vertico, Consult, Marginalia). It's incredibly fast and follows the Unix philosophy.
...
To avoid restarting Emacs every time you change your Org config, we define a function that tangles the file and reloads the resulting elisp.
